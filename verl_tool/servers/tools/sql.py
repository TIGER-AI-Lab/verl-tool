...
class SqlTool(BaseTool):
    tool_type = "sql"
    timeout = TIMEOUT
    stop_tokens = ["```output", "<output>", "<tool_call>", OBS_START]
    ...
    def parse_action(self, action: str) -> Tuple[str, bool]:
        ...
        code = re.findall(r"({SQL_START}.*?{SQL_END})".format(SQL_START=SQL_START, SQL_END=SQL_END), action, re.DOTALL)
    ...
    def conduct_action(self, trajectory_id, action, extra_field):
        ...
        if SQL_START not in parsed_action:
            ...
            observation = f"{OBS_START}Code Extraction Error: code block not detected.{OBS_END}"
            execution_result = ""
            done = False
            valid = False
            correctness = 0.0
            ...
            db_path = extra_field.get("db_path", None)
            ...
            if not correctness:
                ...
                # observation = f"```output\nExecution Result:\n{execution_result}\n```\n"
                observation = f"{OBS_START}Execution Result:\n{execution_result}\n{OBS_END}"
                done = True
