...
You MUST analyze the audio systematically before answering. Follow this process:
1. First, identify what information you need from the audio(s)
2. Use the relevant tool to get useful information from the audio(s)
3. Based on observations, formulate your answer
4. Provide your final answer in \\boxed{} format

You have access to the following tool to analyze audio segments:

<available_tools>
{"type": "function", "function": {"name": "audio_crop", "description": "Crop the audio based on the time window.", "parameters": {"type": "object", "properties": {"time_window": {"type": "array", "description": "A tuple of two numbers indicating the start and end time (in seconds) of the audio segment to crop.", "items": {"type": "number"}}, "target_audio": {"type": "number", "description": "The index of the audio to crop. Index from 1 to the number of audios. Choose 1 to operate on original audio."}}, "required": ["time_window", "target_audio"]}}}
</available_tools>

TOOL USAGE GUIDELINES:
- Use audio_crop when you need to analyze specific temporal segments or patterns
- For questions about trends, counts, or comparisons across time, examine multiple segments
- For multi-audio questions, analyze each audio separately
- Always use tools BEFORE making assumptions about audio content

To invoke the tool, use this EXACT format (ensure proper JSON formatting between <tool_call> and </tool_call>):
<tool_call>
...
RESPONSE STRUCTURE:
1. **Think**: Given the audio, question and possible choices, expand on your reasoning step by step.
2. **Action**: Use relevant tools to get more evidence (useful information) from the audio(s)
3. **Answer**: Put your final answer in \\boxed{exact_answer_text}"""
# -------------------
