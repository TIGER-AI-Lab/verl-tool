#!/bin/bash
#SBATCH --job-name=vt_rb_skysql
#SBATCH --output=logs/vt_rb_skysql_%j/%a.out
#SBATCH --error=logs/vt_rb_skysql_%j/%a.err
#SBATCH --time=180
...
########################################################
CONTAINER=${1:-"/lustre/fsw/portfolios/llmservice/users/dongfuj/images/verltool.sqsh"}
MOUNTS=${2:-"/lustre"}
CONTAINER_WORKDIR=${3:-"/lustre/fsw/portfolios/llmservice/users/dongfuj/Workspace/rebuttal/verl-tool"}
...
val_data=[$CONTAINER_WORKDIR/data/${dataset_name}/test_spider_dev.parquet,\
$CONTAINER_WORKDIR/data/${dataset_name}/test_spider_dk.parquet,\
$CONTAINER_WORKDIR/data/${dataset_name}/test_spider_test.parquet,\
$CONTAINER_WORKDIR/data/${dataset_name}/test_spider_syn.parquet,\
$CONTAINER_WORKDIR/data/${dataset_name}/test_spider_realistic.parquet]

model_name=$CONTAINER_WORKDIR/models/qwen2.5_coder_7b_instruct
...
max_token_len_per_gpu=$((max_token_len_per_gpu < 8192 ? 8192 : max_token_len_per_gpu))
do_offload=False
...
run_name_postfix="-no-retok"
run_name="${reward_manager}-${strategy}-${n_nodes}nodes-${model_pretty_name}-${rl_alg}-n${n}-b${batch_size}-t${temperature}-lr${lr}${run_name_postfix}"
checkpoint_dir=$CONTAINER_WORKDIR/checkpoints/skysql/${run_name}
...
srun $COMMON_SRUN_ARGS \
    ...
    bash -c "
        ...
        python -m verl_tool.servers.serve \
            ...
            --use_ray=True 2>&1 \
            --log_level debug \
            | tee -a $LOG_DIR/tool-server-console.log
    " &
