set -x
dataset_name=dapo_17k # or math_torl_offical to use torl training data
train_data=$(pwd)/data/${dataset_name}/train.parquet
val_data=[$(pwd)/data/${dataset_name}/aime24_test.parquet,\
$(pwd)/data/${dataset_name}/aime25_test.parquet]

model_name=$(pwd)/models/qwen2.5_math_7b
...
enable_mtrl=False
run_name_postfix="-acc-only-${max_turns}-turns-debug"

if [ "$enable_agent" = "True" ]; then
    run_name="math_tir-dapo-${strategy}-agent-${model_pretty_name}-${rl_alg}-n${n}-b${batch_size}-t${temperature}-lr${lr}${run_name_postfix}"
else
    run_name="math_tir-dapo-${strategy}-${model_pretty_name}-${rl_alg}-n${n}-b${batch_size}-t${temperature}-lr${lr}${run_name_postfix}"
fi

checkpoint_dir=$(pwd)/checkpoints/math_tir/${run_name}
...
action_stop_tokens_file="$(pwd)/tmp/verl_tool_action_stop_tokens_$$.txt"
...
python -m verl_tool.servers.serve --host $host --port $port --tool_type "ipython_code" --workers_per_tool 512 --use_ray=True &
server_pid=$!
