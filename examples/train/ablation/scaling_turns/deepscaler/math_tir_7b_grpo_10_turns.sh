...
if [ "$enable_agent" = "True" ]; then
    run_name="math_tir-deepscaler-${strategy}-agent-${n_nodes}nodes-${model_pretty_name}-${rl_alg}-n${n}-b${batch_size}-t${temperature}-lr${lr}${run_name_postfix}"
else
    run_name="math_tir-deepscaler-${strategy}-${n_nodes}nodes-${model_pretty_name}-${rl_alg}-n${n}-b${batch_size}-t${temperature}-lr${lr}${run_name_postfix}"
fi
