...
model_name=$(pwd)/models/qwen2.5_7b
...
train_max_response_length=161000
...
if [ "$enable_agent" = "True" ]; then
    run_name="simple_tir-${strategy}-agent-${model_pretty_name}-${rl_alg}-n${n}-b${batch_size}-t${temperature}-lr${lr}${run_name_postfix}"
else
    run_name="simple_tir-${strategy}-${model_pretty_name}-${rl_alg}-n${n}-b${batch_size}-t${temperature}-lr${lr}${run_name_postfix}"
fi

checkpoint_dir=$(pwd)/checkpoints/simple_tir/${run_name}
...
tool_server_address=http://$host:$port
...
while [ $wait_count -lt $max_wait ]; do
    if curl -s --max-time 2 $tool_server_address/health > /dev/null 2>&1; then
