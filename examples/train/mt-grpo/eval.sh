...
run_name="mt_grpo_eval_$(basename $MODEL | tr '/' '_')"
...
mkdir -p logs
python -m verl_tool.servers.serve --host $host --port $port --tool_type "python_code" --workers_per_tool 2 > logs/tool_server_eval.log 2>&1 &
server_pid=$!
sleep 5
...
PYTHONUNBUFFERED=1 python3 -m verl_tool.recipe.mt_grpo.main_mt_grpo \
    ...
    trainer.project_name=mt_grpo_eval \
    ...
    trainer.total_training_steps=0 2>&1 | tee logs/eval_mt_grpo.log
