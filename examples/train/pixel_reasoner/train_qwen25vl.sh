set -x
dataset_name=pixel_reasoner/PixelReasoner_RL_Data/no_video_max_2048
...
max_prompt_length=2048 #should be big to avoid any truncation of image tokens which will cause error
...
lr=1e-6
reward_manager=torl # should be okay for simple match
...
ulysses_sequence_parallel_size=4 # set to 1 for normal verl behavior, otherwise it will cause OOM
fsdp_size=-1
...
if [ "$enable_agent" = "True" ]; then
    run_name="pixel-reasoner-${strategy}-agent-${model_pretty_name}-${rl_alg}-n${n}-b${batch_size}-t${temperature}-lr${lr}${run_name_postfix}"
else
    run_name="pixel-reasoner-${strategy}-${model_pretty_name}-${rl_alg}-n${n}-b${batch_size}-t${temperature}-lr${lr}${run_name_postfix}"
fi
