...
# Force Ray to spin up a local head unless the caller overrides it.
export RAY_ADDRESS=${RAY_ADDRESS:-local}
# Domain-specific dataset and run name
VAL_FILE=${VAL_FILE:-"$(pwd)/data/mcp_universe/financial_analysis/test.parquet"}
...
max_action_length=${max_action_length:-2048}
max_obs_length=${max_obs_length:-12288}
...
GW_PORT=${GW_PORT:-8321}

SERVERS=${SERVERS:-"yfinance,calculator"}
